...
static ui_menu_focus_t mode_menu[FUNC_NUM] = {
    {&mode_btn_cool_select,  &mode_btn_cool_unselect},
    {&mode_btn_warm_select,  &mode_btn_warm_unselect},
};

static bool mode_set_layer_enter_cb(void *layer);
static bool mode_set_layer_exit_cb(void *layer);
static void mode_set_layer_timer_cb(lv_timer_t *tmr);

static lv_obj_t *page_parent;
static lv_obj_t *img_mode_icon, *img_mode_bg;
...
lv_layer_t mode_set_layer = {
    .lv_obj_name    = "mode_set_layer",
    .lv_obj_parent  = NULL,
    .lv_obj_layer   = NULL,
    .lv_show_layer  = NULL,
    .enter_cb       = mode_set_layer_enter_cb,
    .exit_cb        = mode_set_layer_exit_cb,
    .timer_cb       = mode_set_layer_timer_cb,
};
...
static void mode_setevent_cb(lv_event_t *e)
{
    ...
    if (LV_EVENT_FOCUSED == code) {
        ...
        if (is_time_out(&time_500ms)) {
            ...
            lv_img_set_src(obj_img_button[last_index], mode_menu[last_index].icon_unselect);
...
static void create_title(lv_obj_t *parent)
{
    ...
    lv_label_set_text(select_name, "模式");
...
void ui_mode_setinit(lv_obj_t *parent)
{
    create_title(parent);
    ...
    lv_obj_set_style_text_color(temp_label, lv_color_hex(COLOUR_WHITE), 0);
    lv_obj_align(temp_label, LV_ALIGN_CENTER, -10, 20);
    lv_obj_set_style_text_font(temp_label, &Helvetica65, 0);
    lv_label_set_text(temp_label, "26");

    img_mode_icon = lv_img_create(page_parent);
    lv_img_set_src(img_mode_icon, &mode_icon_cool);
    lv_obj_align_to(img_mode_icon, temp_label, LV_ALIGN_OUT_RIGHT_BOTTOM, 10, 0);

    lv_obj_t *temp_unit_label = lv_label_create(img_mode_bg);
    ...
    lv_obj_align_to(temp_unit_label, temp_label, LV_ALIGN_OUT_RIGHT_TOP, 5, 0);
    ...
    lv_obj_add_event_cb(page_parent, mode_setevent_cb, LV_EVENT_FOCUSED, NULL);
    lv_obj_add_event_cb(page_parent, mode_setevent_cb, LV_EVENT_KEY, NULL);
    lv_obj_add_event_cb(page_parent, mode_setevent_cb, LV_EVENT_LONG_PRESSED, NULL);
    lv_obj_add_event_cb(page_parent, mode_setevent_cb, LV_EVENT_CLICKED, NULL);
...
static bool mode_set_layer_enter_cb(void *layer)
{
    ...
    if (NULL == create_layer->lv_obj_layer) {
        ...
        ui_mode_setinit(create_layer->lv_obj_layer);
