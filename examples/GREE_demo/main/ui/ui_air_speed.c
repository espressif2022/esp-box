...
static bool air_speed_layer_exit_cb(void *layer);
static void air_speed_layer_timer_cb(lv_timer_t *tmr);

static lv_obj_t *page_parent;
static lv_obj_t *img_fan_icon;
...
lv_layer_t air_speed_layer = {
    .lv_obj_name    = "air_speed_layer",
    .lv_obj_parent  = NULL,
    .lv_obj_layer   = NULL,
    .lv_show_layer  = NULL,
    .enter_cb       = air_speed_layer_enter_cb,
    .exit_cb        = air_speed_layer_exit_cb,
    .timer_cb       = air_speed_layer_timer_cb,
};
...
static void air_speedevent_cb(lv_event_t *e)
{
...
static void create_title(lv_obj_t *parent)
{
    ...
    lv_label_set_text(select_name, "风速");
...
void ui_air_speedinit(lv_obj_t *parent)
{
    create_title(parent);

    img_fan_icon = lv_img_create(page_parent);
    lv_img_set_src(img_fan_icon, &speed_air_fan);
    lv_img_set_pivot(img_fan_icon, 110, 75);//66
    lv_obj_center(img_fan_icon);
    ...
    lv_obj_align(page_bottom, LV_ALIGN_BOTTOM_MID, 0, -5);
    ...
    static lv_coord_t grid_clock_col_dsc[] = {LV_GRID_FR(1), LV_GRID_FR(1), LV_GRID_FR(1), LV_GRID_TEMPLATE_LAST};
    ...
    lv_obj_set_grid_cell(obj_img_button[2], LV_GRID_ALIGN_CENTER, 2, 1, LV_GRID_ALIGN_CENTER, 0, 1);


    lv_obj_add_event_cb(page_parent, air_speedevent_cb, LV_EVENT_FOCUSED, NULL);
    lv_obj_add_event_cb(page_parent, air_speedevent_cb, LV_EVENT_KEY, NULL);
    lv_obj_add_event_cb(page_parent, air_speedevent_cb, LV_EVENT_LONG_PRESSED, NULL);
    lv_obj_add_event_cb(page_parent, air_speedevent_cb, LV_EVENT_CLICKED, NULL);
...
static bool air_speed_layer_enter_cb(void *layer)
{
    ...
    if (NULL == create_layer->lv_obj_layer) {
        ...
        ui_air_speedinit(create_layer->lv_obj_layer);
